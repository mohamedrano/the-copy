...
export async function readFiles(files: File[]): Promise<Result<ProcessedFile[]>> {
  try {
    ...
    if (failures.length > 0) {
      log.warn('⚠️ Some files failed to process', failures, 'FileReaderService');
    }
...
async function processFile(file: File): Promise<ProcessedFile | null> {
  try {
    ...
    log.debug(`✅ Processed file: ${file.name} (${formatBytes(file.size)})`, null, 'FileReaderService');
    return processed;

  } catch (error: any) {
    log.error(`❌ Failed to process file: ${file.name}`, error, 'FileReaderService');
    return null;
  }
}
...
async function extractDocxText(file: File): Promise<string | undefined> {
  try {
    ...
    if (result.value && result.value.trim().length > 0) {
      log.debug(`✅ Extracted text from DOCX: ${file.name}`, null, 'FileReaderService');
      return result.value;
    }

    log.warn(`⚠️ DOCX extraction returned empty: ${file.name}`, null, 'FileReaderService');
    return undefined;
  } catch (error) {
    log.error(`❌ DOCX extraction failed: ${file.name}`, error, 'FileReaderService');
    return undefined;
  }
}
