...
export class ErrorHandler {
  ...
  handleAPIError(
    ...
    return this.handleError(error, {
      ...context,
      action: endpoint || context.action,
      component: 'APIService'
    }, ErrorType.API_ERROR, 'high');
  }
   ...
   */
  getErrorsBySeverity(severity: 'low' | 'medium' | 'high' | 'critical'): DetailedError[] {
  ...
  getErrorStats(): {
    ...
    const bySeverity = ['low', 'medium', 'high', 'critical'].reduce((acc, severity) => {
  ...
  private handleCriticalError(error: DetailedError): void {
    ...
    log.error('Critical error detected - emergency procedures may be needed', error, 'ErrorHandler');
  }

  private handleHighSeverityError(error: DetailedError): void {
    // Could show user notifications for high-severity errors
    log.warn('High severity error - user may need to be notified', error, 'ErrorHandler');
  }

  private handleMediumSeverityError(error: DetailedError): void {
    // Could trigger retry mechanisms or alternative flows
    log.info('Medium severity error - retry mechanisms may be triggered', error, 'ErrorHandler');
  }
