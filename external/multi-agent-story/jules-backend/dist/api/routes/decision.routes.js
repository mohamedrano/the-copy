...
export async function decisionRoutes(fastify, options) {
    const { decisionService } = options;
    // Get final decision for a session
    fastify.get('/sessions/:sessionId/decision', async (request, reply) => {
        try {
    ...
    fastify.get('/decisions/:decisionId/summary', async (request, reply) => {
        try {
            const userId = request.user?.id;
            const { decisionId } = request.params;
            if (!userId) {
