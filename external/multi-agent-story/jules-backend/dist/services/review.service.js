...
export class ReviewService {
    ...
    async executeIndependentReviews(sessionId, ideas, apiKey) {
        try {
    ...
    async executeSingleReview(agent, idea, apiKey) {
        try {
            ...
            const reviewResult = await this.agentService.executeAgent(agent.id, 'review', { idea }, apiKey);
    ...
    parseReviewResponse(response) {
        try {
            ...
            if (result.scores.overall === 5) {
                result.scores.overall = (result.scores.quality +
                    result.scores.novelty +
                    result.scores.impact) / 3;
            }
            // Extract strengths
    ...
    async getReviewsBySession(sessionId) {
