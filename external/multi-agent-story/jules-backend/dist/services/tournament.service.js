...
export class TournamentService {
    prisma;
    agentService;
    constructor(prisma, agentService) {
        this.prisma = prisma;
        this.agentService = agentService;
    }
    async executeTournament(sessionId, reviews, apiKey) {
        try {
            ...
            for (let turnNumber = 1; turnNumber <= 8; turnNumber++) {
                const turn = await this.executeTournamentTurn(tournament.id, turnNumber, advancingIdeas, apiKey);
                turns.push(turn);
    ...
    async selectAdvancingIdeas(reviews) {
        try {
            // Group reviews by idea
            const ideaReviews = new Map();
            ...
            for (const [ideaId, ideaReviewsList] of ideaReviews) {
                if (ideaReviewsList.length > 0) {
                    const averageScore = ideaReviewsList.reduce((sum, review) => sum + review.overallScore, 0) / ideaReviewsList.length;
    ...
    async executeTournamentTurn(tournamentId, turnNumber, ideas, apiKey) {
        try {
            ...
            const tournamentArguments = [];
            // Execute each participating agent
            for (const agent of participatingAgents) {
                const argument = await this.executeAgentArgument(agent, ideas, previousTurns, turnNumber, apiKey);
    ...
    async selectTurnAgents(turnNumber, sessionId) {
        ...
        const agentTypesForTurn = turnStrategies[turnNumber] ||
            ['story_architect', 'realism_critic'];
        const selectedAgents = allAgents.filter(agent => agentTypesForTurn.includes(agent.agentType));
        // If not enough agents found, add more
        if (selectedAgents.length < 2) {
            const remainingAgents = allAgents.filter(agent => !agentTypesForTurn.includes(agent.agentType));
            selectedAgents.push(...remainingAgents.slice(0, 2 - selectedAgents.length));
        }
        return selectedAgents;
    }
    async executeAgentArgument(agent, ideas, previousTurns, turnNumber, apiKey) {
        try {
            ...
            const argumentResult = await this.agentService.executeAgent(agent.id, 'tournament_argument', {
                ideas,
                primaryIdea,
                previousArguments,
                turnNumber
    ...
    parseArgumentResponse(response) {
        try {
            const result = {
                stance: 'neutral',
                reasoning: '',
    ...
    async getTournamentBySession(sessionId) {
    ...
    async getTournamentSummary(tournamentId) {
        ...
        const agentParticipation = new Map();
        const ideaSupport = new Map();
