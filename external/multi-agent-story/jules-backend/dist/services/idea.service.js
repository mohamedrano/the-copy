...
export class IdeaService {
    ...
    async generateIdeas(sessionId, creativeBrief, apiKey) {
        try {
            ...
            const storyArchitectResult = await this.agentService.executeAgent(storyArchitectAgent.id, 'idea_generation', { creativeBrief }, apiKey);
            ...
            const ideas = await this.parseGeneratedIdeas(sessionId, storyArchitectResult.response, creativeBrief);
            // Enhance each idea with Character Development agent
            const enhancedIdeas = await this.enhanceIdeasWithCharacterDevelopment(ideas, sessionId, apiKey);
    ...
    async parseGeneratedIdeas(sessionId, response, creativeBrief) {
        const ideas = [];
        try {
    ...
    findNextMarker(text, startIndex, markers) {
    ...
    parseIdeaSection(section, creativeBrief) {
        // Extract title
    ...
    async enhanceIdeasWithCharacterDevelopment(ideas, sessionId, apiKey) {
        try {
            ...
            for (const idea of ideas) {
                const enhancementResult = await this.agentService.executeAgent(characterDevelopmentAgent.id, 'character_enhancement', { idea }, apiKey);
                if (enhancementResult.success) {
                    // Parse character enhancements from the response
                    const enhancedCharacters = this.parseCharacterEnhancements(enhancementResult.response, idea.characters);
    ...
    parseCharacterEnhancements(response, originalCharacters) {
        try {
            ...
            for (const section of characterSections) {
                ...
                if (lines.length >= 2) {
                    ...
                    const characterIndex = enhancedCharacters.findIndex(char => char.name.includes(name) || name.includes(char.name));
                    if (characterIndex !== -1) {
    ...
    async getIdeasBySession(sessionId) {
