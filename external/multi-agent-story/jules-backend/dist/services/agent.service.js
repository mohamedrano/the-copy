...
export class AgentService {
    ...
    loadAgentGuides() {
        ...
        for (const [agentType, filename] of Object.entries(guideFiles)) {
            try {
                ...
                this.agentGuides.set(agentType, content);
    ...
    async createAgentTeam(sessionId, apiKey) {
        try {
    ...
    async createAgent(sessionId, agentType, apiKey) {
    ...
    async executeAgent(agentId, task, context, apiKey) {
        try {
            ...
            const prompt = PromptBuilder.buildAgentPrompt(agent.agentType, guideContent, context);
    ...
    async executeAgentTeam(sessionId, task, context, apiKey) {
        try {
            ...
            const promises = agents.map(agent => this.executeAgent(agent.id, task, context, apiKey));
    ...
    async getAgentsBySession(sessionId) {
    ...
    async testAgentConnection(agentId, apiKey) {
        try {
