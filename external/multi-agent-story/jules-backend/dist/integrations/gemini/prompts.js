export class PromptBuilder {
    static buildAgentPrompt(agentType, guideContent, context) {
    ...
    static getBasePrompt(agentType, guideContent) {
...
- التقدم: ${context.sessionData.progress?.progress || 0}%`;
    ...
    static getTaskPrompt(agentType, phase) {
        const phaseTasks = {
        ...
        const baseTask = phaseTasks[phase] || "قم بتنفيذ مهمتك المتخصصة";
        const specificTasks = {
    ...
    static buildIdeaGenerationPrompt(creativeBrief) {
...
تأكد من أن كل فكرة مكتملة ومفصلة وقابلة للتطوير.`;
    ...
    static buildReviewPrompt(agentType, idea, guideContent) {
...
### التبرير: [شرح مفصل لتقييمك]`;
    ...
    static buildTournamentPrompt(agentType, idea, previousArguments, guideContent) {
