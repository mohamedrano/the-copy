...
export function validateConfig(): void {
  ...
  // Validate Encryption key length (Fernet keys are 44 chars)
  if (config.encryptionKey.length < 44) {
    throw new Error(
      "ENCRYPTION_KEY must be at least 44 characters long (Fernet key) for production",
    );
  }
}
