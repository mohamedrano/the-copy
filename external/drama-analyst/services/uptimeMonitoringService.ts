...
class UptimeMonitoringService {
  ...
  private init(): void {
    if (typeof window === 'undefined') {
      log.warn('⚠️ Uptime monitoring: window not available', null, 'UptimeMonitoringService');
      return;
    }

    log.info('📊 Initializing Uptime monitoring...', this.config, 'UptimeMonitoringService');
  ...
  private performPerformanceCheck(): void {
    try {
      ...
      log.error('❌ Performance check failed', error, 'UptimeMonitoringService');
    }
  }
  ...
  private calculatePerformanceScore(): number {
    ...
    if (this.errorCount > 10) score -= 30; // Over 10 errors
    else if (this.errorCount > 5) score -= 15; // Over 5 errors
  ...
  private determineHealthStatus(performanceScore: number, memoryUsage: number): 'healthy' | 'degraded' | 'unhealthy' {
    if (performanceScore < 50 || memoryUsage > 200 || this.errorCount > 20) {
      return 'unhealthy';
    } else if (performanceScore < 75 || memoryUsage > 100 || this.errorCount > 10) {
      return 'degraded';
    } else {
      return 'healthy';
    }
  ...
  private setupPerformanceTracking(): void {
    ...
    ['click', 'keydown', 'scroll'].forEach(eventType => {
      document.addEventListener(eventType, () => {
        ...
        if (interactionTime > 100) { // Slow interaction
  ...
  private trackEvent(eventName: string, eventData: Record<string, any>): void {
    try {
      ...
      // Log to console
      if (this.config.enableConsoleLog) {
        log.debug(`📊 Uptime event: ${eventName}`, eventData, 'UptimeMonitoringService');
      }
    } catch (error) {
      log.error('❌ Failed to track uptime event', error, 'UptimeMonitoringService');
    }
  }

  private reportHealthIssue(): void {
    if (this.config.enableSentry) {
      reportError(new Error(`Health issue detected: ${this.healthMetrics.status}`), {
  ...
  public destroy(): void {
    ...
    log.info('🔒 Uptime monitoring service destroyed', null, 'UptimeMonitoringService');
  }
}
...
export const initUptimeMonitoring = (config?: Partial<UptimeConfig>) => {
  if (uptimeMonitoringService) {
    log.warn('⚠️ Uptime monitoring service already initialized', null, 'UptimeMonitoringService');
