...
class WebVitalsService {
  ...
  private init(): void {
    if (typeof window === 'undefined') {
      log.warn('⚠️ Web Vitals service: window not available', null, 'WebVitalsService');
      return;
    }

    log.info('📊 Initializing Web Vitals monitoring...', this.config, 'WebVitalsService');
    ...
    const observer = new PerformanceObserver((list) => {
      list.getEntries().forEach((entry) => {
        if (entry.entryType === 'resource') {
          ...
          if (resourceEntry.duration > 1000) { // Resources taking longer than 1 second
            const customMetric: CustomMetric = {
              name: 'SlowResource',
  ...
  private initLongTaskMonitoring(): void {
    ...
    try {
      ...
      log.warn('⚠️ Long task monitoring not supported', error, 'WebVitalsService');
    }
  }
    ...
    if (typeof window !== 'undefined' && window.React) {
      ...
      window.React.createElement = (...args) => {
        ...
        requestAnimationFrame(() => {
          ...
          if (measure && measure.duration > 16) { // Components taking longer than one frame
  ...
  private handleMetric(name: string, metric: CustomMetric): void {
    ...
    if (this.config.enableConsoleLog) {
      log.info(`📊 ${name}: ${metric.value.toFixed(2)}${this.getMetricUnit(name)}`, metric.customData, 'WebVitalsService');
    }
  ...
  private sendToGA4(name: string, metric: CustomMetric): void {
    try {
      ...
      if (this.config.debug) {
        log.debug(`📊 GA4 Web Vitals event sent: ${name}`, metric, 'WebVitalsService');
      }
    } catch (error) {
      ...
      if (typeof window !== 'undefined' && (window as any).gtag) {
        try {
          ...
          log.error('❌ Fallback GA4 also failed', fallbackError, 'WebVitalsService');
        }
      }
    }
  }
  ...
  private sendToSentry(name: string, metric: CustomMetric): void {
    try {
      ...
      if (this.config.debug) {
        log.debug(`📊 Sentry Web Vitals event sent: ${name}`, metric, 'WebVitalsService');
      }
    } catch (error) {
  ...
  private sendToCustomEndpoint(name: string, metric: CustomMetric): void {
    ...
    fetch(this.config.customEndpoint, {
      ...
      log.error(`Failed to send metric to custom endpoint`, error, 'WebVitalsService');
    });
  }
  ...
  private measureCustomMetric(name: string, value: number, customData?: Record<string, any>): void {
    const customMetric: CustomMetric = {
      name,
      value,
      delta: value,
  ...
  public getMetric(name: string): CustomMetric | undefined {
    return Array.from(this.metrics.values()).find(metric => metric.name === name);
  }
    ...
    metrics.forEach(metric => {
      switch (metric.name) {
        case 'CLS':
          ...
          break;
        case 'FID':
          if (metric.value > 100) score -= 20;
          else if (metric.value > 50) score -= 10;
          break;
        case 'LCP':
...
export const initWebVitals = (config?: Partial<WebVitalsConfig>) => {
  if (webVitalsService) {
    log.warn('⚠️ Web Vitals service already initialized', null, 'WebVitalsService');
