...
class AnalyticsService {
  ...
  private init(): void {
    if (typeof window === 'undefined') {
      log.warn('⚠️ Analytics service: window not available', null, 'AnalyticsService');
      return;
    }
    ...
    log.info('📊 Google Analytics 4 initialized', { measurementId: this.config.measurementId }, 'AnalyticsService');
  }
  ...
  private trackUserEngagement(): void {
    ...
    const trackScrollDepth = () => {
      ...
      if (scrollDepth > maxScrollDepth) {
        ...
        scrollDepthThresholds.forEach(threshold => {
          if (scrollDepth >= threshold && !triggeredThresholds.includes(threshold)) {
    ...
    let triggeredTimeThresholds: number[] = [];

    const trackTimeOnPage = () => {
      const timeOnPage = Math.round((Date.now() - startTime) / 1000);

      timeThresholds.forEach(threshold => {
        if (timeOnPage >= threshold && !triggeredTimeThresholds.includes(threshold)) {
  ...
  public trackFileUpload(fileType: string, fileSize: number, fileName: string): void {
  ...
  public trackAnalysisComplete(agent: string, duration: number, success: boolean): void {
  ...
  public trackAnalysisError(agent: string, errorType: string, errorMessage: string): void {
  ...
  public setConsent(consentType: 'analytics_storage' | 'ad_storage', consent: boolean): void {
    if (!this.isInitialized) return;
...
export const initAnalytics = (config: Partial<GA4Config>) => {
  if (analyticsService) {
    log.warn('⚠️ Analytics service already initialized', null, 'AnalyticsService');
