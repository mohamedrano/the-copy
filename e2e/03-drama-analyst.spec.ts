...
test('Drama Analyst Application - should load drama analyst page', async ({ page }) => {
...
test('Drama Analyst Application - should have unique page signature', async ({ page }) => {
  await page.goto('/drama-analyst/');
  await page.waitForLoadState('networkidle', { timeout: 15000 });
...
test('Drama Analyst Application - should render without console errors', async ({ page }) => {
  const errors: string[] = [];
  page.on('console', (msg) => {
    if (msg.type() === 'error') {
  ...
  const criticalErrors = errors.filter(e =>
    ...
    !e.includes('unsupported MIME type')
  );
